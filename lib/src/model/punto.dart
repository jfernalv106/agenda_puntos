...
List<Punto> puntoFromJson(String str) => List<Punto>.from(json.decode(str).map((x) => Punto.fromJson(x)));

String puntoToJson(List<Punto> data) => json.encode(List<dynamic>.from(data.map((x) => x.toJson())));
...
class Punto {
  ...
  factory Punto.fromJson(Map<String, dynamic> json) => Punto(
    cantidad: List<int>.from(json["cantidad"].map((x) => x)),
    fecha: json["fecha"],
  ...
  Map<String, dynamic> toJson() => {
   ...
   // "_id": id== null ? null : id,
    ...
    "latitud": latitud,
    "longitud": longitud,
    "ruta":ruta == null ? [] :List<dynamic>.from(ruta.map((x) => x.toJson())),
